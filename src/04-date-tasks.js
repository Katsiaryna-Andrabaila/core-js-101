...
function timeSpanToString(startDate, endDate) {
  ...
  const hours = Math.floor(period / (60 * 60 * 1000)).toString();
  let minutes = '';
  if (Math.floor(period / 60000) !== 60) {
    minutes = Math.floor(period / 60000).toString().slice(-2);
  }
  if (Math.floor(period / 60000) === 60) {
    minutes = '00';
  }
  const seconds = (period / 1000).toFixed(3).toString().slice(-6);
...
function angleBetweenClockHands(date) {
  ...
  return Math.abs(((Math.min(360 - degree, degree)) * Math.PI) / 180);
}
